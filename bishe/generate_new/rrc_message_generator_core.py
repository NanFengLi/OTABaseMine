...
from choice_utils import get_choices
...
class RRCMessageGenerator:
    ...
    def __init__(self, targets, max_recur_depth=0, seed=20, optional=True):
        ...
        random.seed(seed)
        choice_paths = get_choices(self.bb, targets=self.targets)
        self.choice_paths = [(choices[:-1], paths[1:]) for (choices, paths, _) in choice_paths]
        tmp = []
