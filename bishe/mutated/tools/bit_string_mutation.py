...
完全移植自 OTABase rrc_fuzzer.py::mutate_rrc_bit_field()
"""
import math
import random
...
def _field_bits(field) -> str:
    """BIT STRING 字段比特串（去除填充），移植自 OTABase get_field_bits()"""
...
def _constrained_bit_muts(field) -> List[Tuple[str, int]]:
    """受约束 BIT STRING 4 条变异，移植自 OTABase mutate_rrc_bit_field()"""
    ...
    def gen(lval: int, clen: int):
        ...
        delta = len(bits) - len(fld_bits)   # delta 单位：比特（非字节）
        return (bits, delta)
...
def _unconstrained_bit_muts(field) -> List[Tuple[str, int]]:
    """无约束 BIT STRING 变异，移植自 OTABase mutate_rrc_bit_field()"""
    fld_bits = _field_bits(field)
    muts     = []
