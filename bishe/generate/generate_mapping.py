...
"""
待修改这个文件
这是一个用于生成文件引用关系的脚本，目的是找出每个提取出的 .asn 文件到底用了哪些原始的 .txt 定义块。

核心逻辑如下：

输入与输出：

读取 extracted/*.asn（提取后的文件）和 asn1_blocks/*.txt（原始定义块）。
输出一个 JSON，记录“哪个 asn 文件 -> 包含哪些 txt 文件”。
核心匹配算法（defs 模式）：

分块：先将每个原始 .txt 文件按 ASN.1 语法（Name ::= ...）切分成多个独立的“定义片段”。
去格式化：将所有代码的空格、换行、缩进全部压缩，只比对实质内容，忽略排版差异。
包含判定：只要 .asn 文件中
包含了 .txt 切分出的任意一个定义片段，就判定为匹配成功。

解决的问题：
解决 .asn 文件中定义顺序可能被打散、或者中间插入了其他内容，导致直接整文比对无法匹配的问题。
"""
